...
class App {
  ...
  static get POLYFILLS () {
    return {
      ...
      bower: {
        webcomponentsLite: 'bower_components/webcomponentsjs/webcomponents-lite.js',
        customelements: 'bower_components/custom-elements/custom-elements.min.js',
        htmlimports: 'bower_components/html-imports/html-imports.min.js'
      },
  ...
  bootstrap () {
    ...
    // For mobile browsers...
    if (!this.customElementsSupported || !this.htmlImportsSupported) {
      return new Promise((resolve, reject) => {
        // Load polyfills
        return Promise.all([
          // this._lazyLoadScript(App.POLYFILLS.bower.webcomponentsLite),
          this._lazyLoadScript(App.POLYFILLS.bower.customelements),
          this._lazyLoadScript(App.POLYFILLS.bower.htmlimports)
        ])
          .then(_ => {
            // Load HTML imports
            return Promise.all([
